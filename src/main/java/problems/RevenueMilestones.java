...
public class RevenueMilestones {
  ...
  int[] getMilestoneDays(int[] revenues, int[] milestones) {
    ...
    int[] revenuesSum = new int[revenues.length];
    revenuesSum[0] = revenues[0];
    for (int i = 1; i < revenues.length; i++) revenuesSum[i] = revenuesSum[i - 1] + revenues[i];

    int[] output = new int[milestones.length];
    for (int i = 0; i < milestones.length; i++) {
      int left = 0, right = revenuesSum.length - 1;
      while (left < right) {
        int mid = left + (right - left) / 2;
        if (revenuesSum[mid] >= milestones[i]) right = mid;
        else left = mid + 1;
      }
      output[i] = right + 1;
    }
