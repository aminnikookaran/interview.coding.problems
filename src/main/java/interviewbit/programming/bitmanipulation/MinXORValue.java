...
public class MinXORValue {
  ...
  static void insert(int key) {
    ...
    for (int i = INT_SIZE - 1; i >= 0; i--) {
      ...
      if (temp != null && temp.Child[current_bit] == null) temp.Child[current_bit] = new TrieNode();
  ...
  static int minXORUtil(int key) {
    ...
    for (int i = INT_SIZE - 1; i >= 0; i--) {
      ...
      // same bit
      if (temp.Child[current_bit] != null) temp = temp.Child[current_bit];

      // if there is no same bit.then looking for
      // opposite bit
      else if (temp.Child[1 - current_bit] != null) temp = temp.Child[1 - current_bit];
    }
