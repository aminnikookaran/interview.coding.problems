...
public class Q0421MaximumXOROfTwoNumbersInAnArray {
  ...
  public int findMaximumXOR2(int[] nums) {
    ...
    for (int num : nums) {
      ...
      // we can ignore that bit and directly build from the 30th bit
      for (int i = 30; i >= 0; --i) {
        int bit = getIthBit(num, i);

        // if current bit is 0, it will go to children[1]
        // if current bit is 1, it will go to children[0]
        if (curr.children[bit ^ 1] == null) curr.children[bit ^ 1] = new TrieNode();
