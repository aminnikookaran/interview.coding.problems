...
public class NodesInASubtree {
  ...
  int countOfNodes(Node root, int u, char c, String s, boolean count) {
    // Write your code here
    if (root == null) return 0;
    count = count || root.val == u;
    int sum = 0;
    if (count && s.charAt(root.val - 1) == c) sum++;
    for (Node node : root.children) sum += countOfNodes(node, u, c, s, count);
    return sum;
  }
  ...
  int[] countOfNodes(Node root, ArrayList<Query> queries, String s) {
    ...
    for (int i = 0; i < queries.size(); i++) {
      result[i] = countOfNodes(root, queries.get(i).u, queries.get(i).c, s, false);
    }
    return result;
  }
